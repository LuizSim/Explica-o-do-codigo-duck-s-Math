...
// Função responsável por exibir a pontuação final do jogador na tela, chamada "displayFinalScore"
function displayFinalScore() {
    // Atualiza o texto do elemento "feedback" para mostrar o final do jogo e a pontuação final do jogador
    feedback.innerText = `Fim das perguntas! Sua pontuação final é: ${score} pontos`;
    // Define a classe do elemento "feedback" para "feedback-box correct" para aplicar estilos CSS de visualização da pontuação final
    feedback.className = "feedback-box correct";
    // Torna o elemento "feedback" visível na tela, alterando sua propriedade de estilo para 'block'
    feedback.style.display = 'block';

    // Faz uma requisição HTTP GET para o servidor Flask para gerar o arquivo Excel com os resultados do jogador
    fetch('http://127.0.0.1:5000/')
        .then(response => response.json())  // Espera pela resposta do servidor e converte o conteúdo da resposta para um objeto JSON
        .then(data => {
            // Se a resposta do servidor contém uma mensagem indicando que o arquivo Excel foi gerado com sucesso
            if (data.message === "Arquivo Excel gerado com sucesso") {
                // Exibe uma mensagem no console informando que a planilha foi gerada com sucesso, juntamente com o caminho do arquivo
                console.log("Planilha gerada com sucesso:", data.file_path);
            }
        })
        .catch(error => console.error("Erro ao gerar planilha:", error));  // Se ocorrer um erro durante a requisição, exibe uma mensagem de erro no console
}

// Função "sendResponseToServer" é responsável por enviar os dados de cada resposta do jogador ao servidor para registro
function sendResponseToServer(question, correctAnswer, playerAnswer, score) {
    // Faz uma requisição HTTP POST para enviar os dados da resposta para o servidor Flask
    fetch('http://127.0.0.1:5000/', {
        // Define o método da requisição como 'POST', indicando que queremos enviar dados para o servidor
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'  // Define o cabeçalho da requisição, informando que o corpo da requisição estará no formato JSON
        },
        // Converte os dados das respostas do jogador em formato JSON e os adiciona ao corpo da requisição
        body: JSON.stringify({
            question: question,  // A pergunta que foi feita ao jogador
            correct_answer: correctAnswer,  // A resposta correta para a pergunta
            player_answer: playerAnswer,  // A resposta dada pelo jogador
            score: score  // A pontuação do jogador até aquele momento
        })
    })
    .then(response => response.json())  // Converte a resposta do servidor para um objeto JSON
    .then(data => {
        // Exibe no console uma mensagem de sucesso indicando que a resposta foi registrada corretamente no servidor
        console.log('Resposta registrada com sucesso:', data);
    })
    .catch(error => console.error('Erro ao registrar a resposta:', error));  // Se ocorrer um erro durante o envio, exibe uma mensagem de erro no console
}
